...
class Yeelight extends EventEmitter {
  ...
  updateBySSDPMessage (ssdpMessage) {
    ...
    if (this.support) {
      ...
      if (
        supported.indexOf('set_rgb') != -1 ||
        supported.indexOf('set_hsv') != -1
      ) {
        this.type = 'color';
      }
    }
    ...
    if (this.dcEmitted) {
      this.emit('appeared', true);

      this.dcEmitted = false;
    }
  ...
  updatePower (power) {
    this.power =
      power &&
      ('' + power).toLowerCase() != 'off' &&
      ('' + power).toLowerCase() != 'false' &&
      power != '0';
  ...
  parseResponse (res) {
    ...
    splits.forEach(response => {
      ...
      try {
        ...
        this.emit('failed', {
          reason: 'response is not parsable',
          response: response
        });
      }
      ...
      if (json) {
        ...
        if (
          ...
          id &&
          this.messages[id] &&
          this.messages[id].method == 'get_prop'
        ) {
          ...
          if (params.length == values.length) {
            ...
            this.emit('failed', {
              reason:
                'error on parsing get_prop result --> params length != values length',
              response: response
            });
            console.error(
              'error on parsing get_prop result --> params length != values length'
  ...
  sendCommand (method, params) {
    ...
    if (
      (this.support == '' || supportedMethods.indexOf(method) != -1) &&
      params &&
      params.length > 0
    ) {
  ...
  updateState () {
    return this.sendCommand('get_prop', [
      'power',
      'color_mode',
      'ct',
      'rgb',
  ...
  setRGB (rgb, duration) {
    ...
    const params = [number, duration ? 'smooth' : 'sudden', duration || 0];
  ...
  setBright (bright, duration) {
    ...
    const params = [bright, duration ? 'smooth' : 'sudden', duration || 0];
  ...
  setHSV (hsv, duration) {
    ...
    const params = [hue, sat, duration ? 'smooth' : 'sudden', duration || 0];
  ...
  setCT (ct, duration) {
    ...
    const params = [ct, duration ? 'smooth' : 'sudden', duration || 0];
